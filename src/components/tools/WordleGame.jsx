...
const WordleGame = () => {
  ...
  const submitGuess = async () => {
    ...
    try {
      ...
      if (data.win) {
        toast.success(
          `🎉 Congratulations! You guessed the word in ${data.attempt} attempts!`
        );
      ...
      if (error.response) {
        ...
        if (
          ...
          errorMsg.toLowerCase().includes("not a valid word")
        ) {
        ...
        if (
          ...
          error.response.data?.error?.includes("Maximum attempts")
        ) {
          setGameState((prev) => ({
            ...prev,
            isComplete: true,
  ...
  const shareResults = () => {
    ...
    const results = gameState.attempts
      .map((attempt) =>
        attempt.result
          .map((status) => {
            switch (status) {
    ...
    const shareText = `Daily Wordle ${formattedDate}\n\n${results}\n\n${
      gameState.hasWon
        ? `✅ Solved in ${gameState.attemptsMade}/6`
        : "❌ Failed"
        ...
        >
          ...
          <div className="grid grid-cols-1 gap-2 mb-4">
            ...
            {Array.from({ length: 6 }).map((_, attemptIndex) => {
              ...
              const isCurrent =
                attemptIndex === gameState.attemptsMade &&
                !gameState.isComplete;
